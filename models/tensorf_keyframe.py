...
import sys
import os
project_root = '/data/lunengbo/LNB_doc/NVFi'
sys.path.append(project_root)

from models.tensorf_base import TensorBase
from models.base_network import PositionEncoder, SineActivation, BaseMLP
from models.velocity_field import *
from models.Keyframe_net import *
...
from models.tensorf_model_utils import (
    AlphaGridMask,
    raw2alpha,
    alpha2weights,
...
class TensorVMKeyframeTimeKplane(TensorBase):
    def __init__(self, aabb, gridSize, device, near_far, cfg):
        ...
        if self.use_vel:
            ...
            try:
                ...
                self.vel = VelocityAABBSur(self.vel_net, self.aabb, surround).to(device)
                print('VelocityAABBSur')
            except:
                self.vel = VelocityAABB(self.vel_net, eps).to(device)
                print('VelocityAABB')
    ...
    def init_svd_volume(self, device):
        ...
        self.pretrained_model = VGG16XYTZPredictor(input_time_dim=1,output_dim=4).to(device)

        # self.pot_mat=nn.Sequential(nn.Linear(8,256),nn.ReLU(),nn.Linear(256, 128),nn.GELU(),nn.Linear(128, 4)).to(device)
    #     self.pot_mat2=nn.Sequential(nn.Linear(8,128),nn.ReLU(),nn.Linear(128, 128),nn.GELU(),nn.Linear(128, 4)).to(device)
        self.pot_t=nn.Sequential(nn.Linear(8,64),nn.ReLU(),nn.Linear(64, 32),nn.GELU(),nn.Linear(32, 4)).to(device)
    #     self.pot_t1=nn.Sequential(nn.Linear(16,128),nn.ReLU(),nn.Linear(128, 128),nn.GELU(),nn.Linear(128, 4)).to(device)
    #     # self.w = nn.Parameter(torch.tensor(0.7, requires_grad=True))

    # def encode_xyzt_sine(self,xyzt_tensor: torch.Tensor, scale: float = 2 * torch.pi) -> torch.Tensor:
    
    #     x, y, z, t = xyzt_tensor[..., 0], xyzt_tensor[..., 1], xyzt_tensor[..., 2], xyzt_tensor[..., 3]
        
        
    #     def encode_dim(dim):
    #         dim_scaled = dim * scale  
    #         return torch.sin(dim_scaled), torch.cos(dim_scaled)
        
        
    #     sin_x, cos_x = encode_dim(x)
    #     sin_y, cos_y = encode_dim(y)
    #     sin_z, cos_z = encode_dim(z)
    #     sin_t, cos_t = encode_dim(t)
        
        
    #     encoded = torch.stack([sin_x, cos_x, sin_y, cos_y, sin_z, cos_z, sin_t, cos_t], dim=-1)
    #     return encoded

    # def SpatialTemporalPredictor(self,xt1,xt2):
    #     xt1_encode=self.encode_xyzt_sine(xt1)
    #     xt2_encode=self.encode_xyzt_sine(xt2)
        
    #     # xt11=self.pot_t(xt1_encode)
    #     # xt12=self.pot_t(xt2_encode)
    #     # x=torch.cat([xt11,xt12],dim=-1)
    #     x=torch.cat([xt1_encode,xt2_encode],dim=-1)
        
    #     xt_prv=self.pot_t1(x)
        
    #     return xt_prv
    
    # def consistency_loss(self,coord_A, coord_B):
    #     return 1.0 / (1.0 + torch.mean((coord_A - coord_B) ** 2))
    
    # def Unsupervised_Ensemble(self,coord_A,coord_B):
        
    #     # self.w = torch.tensor(0.5, requires_grad=True)  # 初始权重
        
    #     coord_A = coord_A.requires_grad_(True)
    #     coord_B = coord_B.requires_grad_(True)
        
    #     x=torch.cat([coord_A,coord_B],dim=-1)
    #     # p1=self.pot_mat2(coord_A )
    #     # p2=self.pot_mat2(coord_B)
    #     combined_coord=self.pot_mat2(x)
    #     # mse_loss = torch.mean((p1 - p2) ** 2)
    #     # combined_coord = self.w * p1 + (1 - self.w) * p2
    #     # print("w:",self.w)
...
#################################################################################################################
    ...
    def get_optparam_groups(self, lr_init_spatialxyz=0.02, lr_init_network=0.001):
        grad_vars = [{'params': self.density_plane_space, 'lr': lr_init_spatialxyz},
                     ...
                     {'params': self.pretrained_model.parameters(), 'lr': lr_init_network},
                    #  {'params': self.pot_mat.parameters(), 'lr': lr_init_network},
                    #  {'params': self.pot_mat2.parameters(), 'lr': lr_init_network},
                     {'params': self.pot_t.parameters(), 'lr': lr_init_network}]
                    #  {'params': self.pot_t1.parameters(), 'lr': lr_init_network}]
                    #  {'params': self.w, 'lr': lr_init_network}]
        if isinstance(self.renderModule, torch.nn.Module):
...
########################################################################################################################
    def integrate_pos(self, pos_init, t, base_times):
        ...
        while unfinished.any():
            # print("unfinished shape:", unfinished.shape)
            # get time step
            ...
            # xyzt1_prev = torch.cat([xyz_prev[unfinished]+1, t_curr[unfinished]+1], dim=-1)
            # xyzt_prv1=self.pot_t(xyzt_prev)
            # pot_prv=xyzt_prev[..., :3]
            # t_prv=xyzt_prev[..., 3:]
            # print("t_curr:",t_curr.shape)
            # print("base_times[unfinished]:",base_times[unfinished].shape)
            # print("t_curr[unfinished]:",t_curr[unfinished].shape)
            # base_times = torch.ones_like(t)
            key_idx = torch.searchsorted(base_times[unfinished], t_curr[unfinished], right=True) - 1  
            
            next_key_idx = key_idx + 1
            
            key_idx = torch.clamp(key_idx, 0, len(base_times)-1)
            next_key_idx = torch.clamp(next_key_idx, 0, len(base_times)-1)
            
            t_prev = base_times[key_idx]  
            t_next = base_times[next_key_idx]  
            xyz_prev_key = xyz_prev[key_idx]  
            xyz_next_key = xyz_prev[next_key_idx]  

            xyzt_prev = torch.cat([xyz_prev[unfinished], t_curr[unfinished]], dim=-1)
            
            xyzt_key_prev = torch.cat([xyz_prev_key, t_prev], dim=-1)  
            xyzt_key_next = torch.cat([xyz_next_key, t_next], dim=-1)  
            # print("xyzt_prev:",xyzt_prev.shape)
            
            
            xyzt_key_prev = xyzt_key_prev.squeeze(1)
            xyzt_key_next = xyzt_key_next.squeeze(1)
            # print("xyzt_key_prev:",xyzt_key_prev.shape)
            # print("xyzt_key_next:",xyzt_key_next.shape)
            
            xyzt_combined = torch.cat([xyzt_key_prev, xyzt_key_next], dim=-1)
            xyzt_combined = self.pot_t(xyzt_combined)
            # print(f"xyzt_combined:",xyzt_combined.shape)
            xyz_combined=xyzt_combined[..., :3]
            t_combined=xyzt_combined[..., 3:]
            # print("xyzt_combined:",xyzt_combined.shape)
            # print(type(pretrained_model)) 
            pred_xyz_batch = self.pretrained_model(xyz_combined,t_combined).to(device)
            ...
            # p_prv=torch.cat([p_mid, pot_prv], dim=-1)
            # t_prv1=torch.cat([t_prv, t_mid], dim=-1)

            # pt_mid=self.pot_t1(torch.cat([p_prv,t_prv1], dim=-1))
            # import pdb
            pt_mid = torch.cat([t_mid, p_mid], dim=-1)
            # pt_mid1=self.SpatialTemporalPredictor(xyzt_prev,xyzt_combined)
            # pt_mid = self.Unsupervised_Ensemble(pt_mid,pt_mid1)
            # pt_mid1 = pt_mid1[...,:3]
            # prv_loss = pt_mid - pred_xyz_batch
            # prv_loss = torch.mean((pt_mid - pred_xyz_batch) ** 2)
            # print("prv_loss:",prv_loss)
            # pt_pre = torch.cat([pt_mid,pred_xyz_batch],dim=-1)
            # pt_mid = self.pot_mat(pt_pre)
            pt_mid = (pt_mid +pred_xyz_batch)/2
            # pdb.set_trace()
            xyz_cur = xyz_prev[unfinished] - dt * self.vel(pt_mid)
            # xyz_cur = xyz_prev[unfinished] - dt * self.vel(pred_xyz_batch)
            # xyz_cur = self.Unsupervised_Ensemble(xyz_cur,pt_mid1)
             # xyz_cur =self.pot_mat(xyz_cur)
             # print("xyz_cur:",xyz_cur.shape)
