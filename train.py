...
def main(config_args):
    ...
    gscaler = torch.amp.GradScaler(device='cuda', enabled=train_fp16)
    ...
    for epoch in pbar:
        ...
        nvfi.train().to(device)
        renderer.train().to(device)

        # with torch.cuda.amp.autocast(enabled=train_fp16):
        with torch.amp.autocast(device_type='cuda', enabled=train_fp16):
            ...
            if not config_args.static:
                ...
                pose = all_poses['train'][idx].to(device)
            ...
            if config_args.static:
                ...
                pose_init = all_poses['init'][idx].to(device)
                ...
                pose_key = all_poses['train'][idx].to(device)
        ...
        if epoch % cfg.experiment.validate_every == 0 or epoch == cfg.experiment.train_iters - 1:
            with torch.no_grad():
                ...
                if config_args.static:
                    ...
                    pose = all_poses['val'][idx].to(device)
                    t_list = [0]
                else:
                    idx = np.random.randint(counts['val'])
                    target = all_targets['val'][idx].to(device)
...
def load_model_checkpoint(cfg, checkpoint, device):
    ...
    try:
        ...
        pass
    nvfi.load_state_dict(checkpoint["model_state_dict"],strict=False)
    renderer = Renderer(
...
#     print("test_poses 类型:", type(test_poses))
#     print("test_targets 类型:", type(test_targets))
#     print("test_times 类型:", type(test_times))
    
#     mask = [t <= 0.75 for t in test_times] 
#     # filtered_poses = test_poses[mask]       # 或 [test_poses[i] for i in range(len(test_poses)) if mask[i]]
#     # filtered_targets = test_targets[mask]   # 同理
#     # filtered_times = test_times[mask]
#     filtered_poses = [pose for pose, m in zip(test_poses, mask) if m]
#     # filtered_targets = [target for target, m in zip(test_targets, mask) if m]
#     filtered_targets = test_targets[mask]
#     filtered_times = [t for t, m in zip(test_times, mask) if m]
#     print(f"filtered_targets:",filtered_targets.shape)
...
#         for idx in tqdm.trange(len(filtered_poses)):
#             # pose = test_poses[idx]
#             # target = test_targets[idx]
#             # t = test_times[idx]
#             pose = filtered_poses[idx]
#             target = filtered_targets[idx]
#             t = filtered_times[idx]
#             # pdb.set_trace()
#             # print("t:",t)
#             camera = Camera(pose, H, W, focal, target, cfg.dataset.near, cfg.dataset.far)
...
if __name__ == '__main__':
    ...
    if config_args.eval_val:
        ...
        render_test_evaluation(config_args, checkpoint, 'test')
